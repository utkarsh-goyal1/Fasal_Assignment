...
function Banner({ name, language, releaseDate, genre, ratings, votes }) {
    return (
        <div className={`h-[350px] md:h-[500px] bg-no-repeat bg-cover bg-center relative w-full`}>
                ...
                <div className='flex flex-col justify-center w-full py-12 px-8'>
                    ...
                    <div className='my-2'>
                        <div className='flex gap-1'>
                            {Array.isArray(language) ? language.map((data, index) => (
                                <div className='bg-gray-300 text-gray-900 w-fit rounded-sm text-md font-light px-2 my-3' key={index}>{data}</div>
                            )) : language.split(',').map((data, index) => (
                                <div className='bg-gray-300 text-gray-900 w-fit rounded-sm text-md font-light px-2 my-3' key={index}>{data.trim()}</div>
                            ))}
                        </div>
                        <div>
                            <h1 className='text-gray-200'> {genre} · {releaseDate} · {votes}</h1>
                        </div>
                    </div>
                </div>
            </div>
